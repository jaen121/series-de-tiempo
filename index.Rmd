...
```{r}
# Gráfico de la serie
ts.plot(df_maiz_filtradots1, main = "PRECIO DEL MAIZ POR DÍA", col = "darkgreen")
...
adf.test(df_maiz_filtradots1, alternative =  c("stationary", "explosive") )

```
Tenemos un valor de p-value>0.05 no rechazamos la hipótesis nula de que la serie no sea estacionaria.

Validando cuantas veces se debe diferenciar para tener una serie estacional

```{r}
ndiffs(df_maiz_filtradots1)
...
dif1_maiz <- diff(df_maiz_filtradots1)
...
\begin{itemize}
    \item \( y(t) \): es el valor de la serie temporal en el tiempo \( t \).
    \item \( g(t) \): Representa la tendencia de la serie
    \item \( s(t) \): Representa el componente de estacionalidad
    \item \( h(t) \): Representa el componente cíclico, captura los efectos de las vacaciones
    \item \( \epsilon_t \): representa el ruido blanco
\end{itemize}
