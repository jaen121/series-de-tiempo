...
Y1 <- as.ts(df_maiz_filtrado$Último,start = c(2000, 1), frequency=261)
...
```
El grafico anterior muestra tanto los datos reales como las predicciones de un modelo ajustado, donde:
...
```{r echo= TRUE, message=FALSE, warning=FALSE, fig.align='center'}
#posterior desnormalizaremos los datos:

mod1 <- predictions*(max(Z)-min(Z))+min(Z)
mod1

```
Ahora se grafica  la representación de los valores predichos para los siguientes días.
...
```{r echo= TRUE, message=FALSE, warning=FALSE, fig.align='center'}
# Supongamos que ya has creado la serie de tiempo S y la has normalizado
