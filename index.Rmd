...
```{r echo= TRUE, message=FALSE, warning=FALSE, fig.align='center'}
# Instalar y cargar el paquete skimr si no está instalado
...
```{r echo= TRUE, message=FALSE, warning=FALSE, fig.align='center'}
# Instalar y cargar el paquete xts si no está instalado
...
#ordenar la serie
df_maiz_filtrado_ordenado <- df_maiz_filtrado %>%
  arrange(Fecha)


# Crea una serie de tiempo con la función ts
df_maiz_filtradots <- ts(df_maiz_filtrado_ordenado$Último, start = c(2000, 1), frequency=261.25)
df_maiz_filtradots
...
Se gráfica la serie de tiempo por días desde 03 de Enero del año 2000 hasta el 19 de abril del año 2024, excluyendo los fines de semana.
...
Por buen manejo metodológico, se opta por realizar una transformación a la base de datos para poder realizar predicciones. El método elegido para realizar esta transformación es tomando las diferencias de los datos.
...
```
Según el resultado se debe diferenciar una vez para tener la serie estacional. Sin embargo, esto aumenta significatiamente el esfuerzo computacional, por lo que se decidió diferenciar dos veces.
...
La salida anterior presenta los intervalos de confianza de los pronósticos, del valor del precio del maíz.
...
- *MSE (Mean Squared Error)*: La varianza promedio de los errores de predicicón es de *161.8*.
- *RMSE (Root Mean Squared Error)*: La raíz cuadrada del MSE es de *12.72*
- *MAE (Mean Absolute Error):* El error absoluto promedio es de *8.2*
...
Las redes neuronales de Elman, también conocidas como redes neuronales recurrentes de Elman, son un tipo específico de red neuronal recurrente (RNN) desarrollada por Jeffrey Elman en 1990. Estas redes están diseñadas para manejar datos secuenciales y temporales, lo que las hace adecuadas para tareas como el procesamiento de lenguaje natural, el reconocimiento de patrones y la predicción de series temporales. Algunas de  sus características y funcionamiento es el siguiente:

## Estructura de una Red Neuronal de Elman

1. **Capas de la Red**:
   + **Capa de Entrada**: Esta capa recibe el input de datos secuenciales. Cada neurona en esta capa corresponde a una característica del vector de entrada.
   + **Capa Oculta**: La capa oculta procesa la información recibida de la capa de entrada y contiene una memoria interna llamada **capa de contexto**.
   + **Capa de Salida**: Esta capa produce el resultado final basado en la activación de las neuronas de la capa oculta.

2. **Capa de Contexto**:
   + **Unidades de Contexto**: La capa de contexto almacena la activación de la capa oculta en el tiempo \( t-1 \). Estas unidades se retroalimentan a la capa oculta en el siguiente paso temporal \( t \).
   + **Función de Memoria**: Las unidades de contexto permiten que la red de Elman tenga una memoria a corto plazo, lo que le permite procesar y aprender dependencias temporales.

## Funcionamiento de la Red

+ **Propagación Hacia Adelante**: En cada paso temporal, la red de Elman procesa la entrada actual y la activación de la capa de contexto (que contiene la activación previa de la capa oculta).
  - La capa oculta recibe tanto la entrada actual como las unidades de contexto.
  - La activación de la capa oculta se calcula aplicando una función de activación (generalmente una función sigmoide o ReLU).
  - La salida de la capa oculta se pasa a la capa de salida para producir el resultado final.
  - La activación de la capa oculta también se almacena en las unidades de contexto para el siguiente paso temporal.

+ **Propagación Hacia Atrás (Entrenamiento)**: El entrenamiento se realiza mediante retropropagación a través del tiempo (BPTT), un algoritmo que extiende el algoritmo de retropropagación para manejar dependencias temporales.
  - El error se propaga hacia atrás desde la capa de salida a través de las conexiones recurrentes hasta las capas anteriores.
  - Los pesos de las conexiones se ajustan para minimizar el error de predicción.

## Ventajas y Aplicaciones

+ **Modelado de Secuencias Temporales**: Las redes de Elman son efectivas para tareas que requieren entender secuencias y dependencias temporales, como la predicción de series temporales, el reconocimiento de voz y el procesamiento del lenguaje natural.
+ **Simplicidad**: En comparación con otras RNN más complejas, las redes de Elman tienen una estructura relativamente simple y son más fáciles de entrenar y ajustar.
+ **Memoria a Corto Plazo**: La capa de contexto permite que la red de Elman retenga información de pasos temporales previos, mejorando su capacidad para aprender patrones temporales.

## Limitaciones

+ **Memoria a Largo Plazo**: Las redes de Elman pueden tener dificultades para aprender dependencias a largo plazo debido a problemas como el desvanecimiento del gradiente.
+ **Capacidad Limitada**: Aunque son efectivas para tareas simples, pueden no ser suficientes para problemas complejos que requieren modelado de dependencias a largo plazo, donde arquitecturas más avanzadas como LSTM o GRU son más adecuadas.

En resumen, las redes neuronales de Elman son un modelo fundamental en el campo de las redes neuronales recurrentes, con aplicaciones útiles en diversas áreas que involucran datos secuenciales y temporales.

El modelo de las redes neuronales de Elman se puede describir de manera formal mediante una serie de ecuaciones que representan las activaciones y actualizaciones de las capas en cada paso temporal. Aquí se presenta el modelo matemático:

## Notación

+ \( \mathbf{x}(t) \): Vector de entrada en el tiempo \( t \).
+ \( \mathbf{h}(t) \): Vector de activación de la capa oculta en el tiempo \( t \).
+ \( \mathbf{y}(t) \): Vector de salida en el tiempo \( t \).
+ \( \mathbf{h}(t-1) \): Vector de activación de la capa oculta en el tiempo \( t-1 \) (capa de contexto).
+ \( \mathbf{W}_{xh} \): Matriz de pesos entre la capa de entrada y la capa oculta.
+ \( \mathbf{W}_{hh} \): Matriz de pesos entre la capa de contexto (activación de la capa oculta en \( t-1 \)) y la capa oculta.
+ \( \mathbf{W}_{hy} \): Matriz de pesos entre la capa oculta y la capa de salida.
+ \( \mathbf{b}_h \): Vector de sesgo de la capa oculta.
+ \( \mathbf{b}_y \): Vector de sesgo de la capa de salida.
+ \( \sigma \): Función de activación, típicamente una función sigmoide o ReLU.

## Ecuaciones del Modelo

1. **Activación de la Capa Oculta**:
   La activación de la capa oculta en el tiempo \( t \) se calcula como:
   \[
   \mathbf{h}(t) = \sigma(\mathbf{W}_{xh} \mathbf{x}(t) + \mathbf{W}_{hh} \mathbf{h}(t-1) + \mathbf{b}_h)
   \]
   Aquí, \( \mathbf{W}_{xh} \mathbf{x}(t) \) representa la contribución de la entrada actual, \( \mathbf{W}_{hh} \mathbf{h}(t-1) \) representa la contribución de la activación previa de la capa oculta (memoria), y \( \mathbf{b}_h \) es el sesgo.
...
2. **Cálculo de la Salida**:
   ...
   \]
   Aquí, \( \mathbf{W}_{hy} \mathbf{h}(t) \) representa la contribución de la activación de la capa oculta al cálculo de la salida, y \( \mathbf{b}_y \) es el sesgo.

## Resumen del Funcionamiento

En cada paso temporal \( t \):
1. Se recibe un nuevo vector de entrada \( \mathbf{x}(t) \).
2. Se actualiza la activación de la capa oculta \( \mathbf{h}(t) \) utilizando la entrada actual y la activación previa de la capa oculta \( \mathbf{h}(t-1) \).
3. Se calcula la salida \( \mathbf{y}(t) \) basada en la activación de la capa oculta \( \mathbf{h}(t) \).

## Diagrama del Modelo

Un diagrama de la red neuronal de Elman puede ayudar a visualizar el flujo de datos:

```
      Entrada (x(t))  ---->  [Capa Oculta (h(t))]
                                ^      |
                                |      v
        Contexto (h(t-1))  ---> [Capa Oculta (h(t))]
                                |
                                v
                           [Salida (y(t))]
```
...
En el diagrama:
+ La capa de entrada envía datos a la capa oculta.
+ La capa oculta tiene una conexión recurrente que le permite recibir información de su propio estado anterior.
+ La capa oculta envía datos a la capa de salida, produciendo la salida de la red.

Este modelo permite que la red neuronal de Elman mantenga una memoria a corto plazo de las entradas anteriores, lo que es crucial para tareas que involucran datos secuenciales y temporales.

## Realización del modelo 

Cargamos los paquetes y normalizamos la serie Utilizamos la fórmula estándar de normalización $\frac{{Y - \min(Y)}}{{\max(Y) - \min(Y)}}$ para obtener $Y1_{\text{normalizado}}$. Luego, utilizamos \texttt{plot()} para graficar la serie de tiempo normalizada.
...
Y1 <- as.ts(df_maiz_filtrado_ordenado$Último,start = c(2000, 1), frequency=261)
...
plot(S, main = "Serie de Tiempo Normalizada", xlab = "Tiempo", ylab = "Valor Normalizado")

```

Dividimos la serie de tiempo en el 75% de datos de entrenamiento y el 25% en datos de testeo.
...
set.seed(42)
fit<-elman(inputs[train],outputs[train],size=10,learnFuncParams=c(0.1),
                  maxit=10000)




plotIterativeError(fit, main = "Error iterativo para  10 neuronas")
```
En la gráfica anterior se ve como evoluciona el error de la red con el numero de iteraciones para los parámetros expuestos.Donde se observa la convergencia rápida hacia cero, se espera entonces un ajuste rápido del modelo.
...
El gráfico anterior muestra tanto los datos reales como las predicciones de un modelo ajustado, donde:

+ Líneas Negras corresponden a la serie de tiempo de los valores reales y, que se obtienen de los datos de salida (outputs) excluyendo los datos de prueba (test). La línea negra representa la evolución temporal o secuencial de estos valores reales.

+ Líneas Rojas representan las predicciones del modelo ajustado (fit) para los mismos datos de entrada utilizados para generar y, es decir, inputs[-test, lo que nos permite comparar visualmente las predicciones del modelo con los valores reales.

Ahora gracias al efecto memoria vamos a adelantarle a la serie al menos en un valor con una precisión muy buena. Para ello volveremos a introducir los datos de entrenamiento.
...
Ahora  desnormalizamos los datos
...
```
Ahora se gráfica  la representación de los valores predichos para los siguientes días.
...
El modelo de red neuronal de Jordan es una variante de las RNN que tiene conexiones hacia atrás de las capas ocultas a sí mismas. Esto significa que la salida de una capa oculta en un momento dado se convierte en una entrada en el siguiente paso de tiempo. Esta estructura le otorga a la red una especie de "memoria de corto plazo", lo que puede ser útil para capturar dependencias temporales complejas en los datos.

El funcionamiento corresponde a que cuando se aplica un modelo de Jordan a datos de series de tiempo, la red neuronal aprende a predecir valores futuros basándose en los valores pasados y presentes de la serie temporal. Por ejemplo, si tienes datos de temperatura registrados cada hora, la red aprenderá patrones y tendencias en los datos para hacer predicciones sobre cómo será la temperatura en la siguiente hora.
...
Hacemos las serie de tiempo promediandola por año sin tener en cuenta sábados y domingos.


```{r echo= TRUE, message=FALSE, warning=FALSE, fig.align='center'}

# Convertir S a una serie de tiempo con frecuencia anual
S_ts <- ts(S, start = c(2000, 1), frequency = 262)  # Aquí asumo que F=261 es la frecuencia original

# Calcular el promedio anual de los datos normalizados
S_anual <- aggregate(S_ts, nfrequency = 1, FUN = mean)

# Graficar la serie de tiempo normalizada de manera anual
plot(S_anual, main = "Serie de Tiempo Anual Normalizada", xlab = "Año", ylab = "Valor", type = "l")
