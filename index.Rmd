...
```{r}
# Gráfico de la serie
ts.plot(df_maiz_filtradots, main = "PRECIO DEL MAIZ POR DÍA", col = "darkgreen")
...
dif_dia <- diff(df_maiz_filtradots)

#variación
variacion_dia <- diff(df_maiz_filtradots) / lag(df_maiz_filtradots, -1)

#tranformación logaritmica
df_maiz_log <- log(df_maiz_filtradots)


# Gráfico de la serie
ts.plot(dif_dia, main = "Diferencia", col = "darkblue")


ts.plot(variacion_dia, main = "Variación", col = "darkred")


ts.plot(df_maiz_log, main = "df_maiz_log", col = "darkgray")
...
print('------------------------')
print('Prueba Estacionariedad Diferencia diaria')
adf.test(dif_dia)

print('------------------------')
print('Prueba Estacionariedad Variación diaria')
adf.test(variacion_dia)


print('------------------------')
print('Prueba Estacionariedad df_maiz_log')
adf.test(df_maiz_log)
...
acf(df_maiz_filtradots, lag.max =  36)
pacf(df_maiz_filtradots, lag.max =  36)
```

```{r}
par(mfrow = c(1,1))

acf(dif_dia, lag.max =  36)
pacf(dif_dia, lag.max =  36)
```
...
acf(variacion_dia, lag.max =  36)
pacf(variacion_dia, lag.max =  36)
```
...
#mod <- auto.arima(df_maiz_filtradots1)
#mod
```

```{r}

mod_arima_auto <- auto.arima(dif_dia)
...
modelo_arima1<- arima(dif_dia, order=c(1, 0, 8))

# Residuos del modelo ARIMA
residuos_mod_arima1 <- residuals(modelo_arima1)

# Gráfico de los residuos
plot(residuos_mod_arima1)
...
```{r}
#pronostico<- predict(mod, n.ahead = 261, prediction.interval = T, level = 0.95)
#pronostico
#plot(pronostico,main="Pronóstico con auto.arima",xlab="Tiempo",ylab="Precio")
#plot(mod, pronostico)
#grid()
```
...
# Calcular el error de pronóstico
error_auto <- test - predicciones$pred
